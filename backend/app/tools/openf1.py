...
Brief path ``backend/tools/openf1.py`` maps to ``app.tools.openf1`` in this repo.
"""
...
async def fetch_team_radio(
    *,
    ...
    client = _client(openf1)
    wanted = {int(n) for n in (driver_numbers or []) if n is not None}
    try:
    ...
    for row in rows or []:
        ...
        try:
            ...
            number_int = None
        if wanted and number_int not in wanted:
            continue
        cleaned.append(
            {
                "date": row.get("date"),
                "driver_number": number_int,
