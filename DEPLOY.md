# Deploy — Apex F1 Suite

Two services: **Vercel (Next.js)** and **Railway (FastAPI)**. Chat, steward, and a cold dashboard can take tens of seconds. Do not put FastAPI on Vercel serverless. Never commit `.env`.

Feature inventory: [FEATURES.md](./FEATURES.md).

```
Browser ── /season/* , /steward, /about ──► Vercel (static + client JS)
...
| `PINECONE_API_KEY` | Steward FIA RAG. Optional; without it teaching Markdown / keyword fallback |
| `PINECONE_INDEX` / `PINECONE_NAMESPACE` | Defaults in `backend/.env.example` |
...
3. Switch **Driver Assets** (`?tab=driver`) → top-5 FER cards + teammate scatter.
4. Finance still works if you type e.g. “best cost-per-point in 2024”.
5. Open `/steward` → Spa demo auto-runs; dossier shows citation cards + evidence checklist (OpenRouter + Pinecone needed for the full live path).
6. `GET /health` on Railway → `status: ok`. After Supabase is wired, `facts_backend` is `supabase` and `facts_count` is ~40+.
7. Manufacturer **Race wins** should track the season (not a handful of GPs from one Ergast page).

Supabase one-time SQL and seed behavior: [FACTS.md](./FACTS.md). Feature map: [FEATURES.md](./FEATURES.md).
